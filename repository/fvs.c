...
//        printf("Lim_min: %d -- Lim max: %d\n", lim_min, lim_max);
        

        // Calculate Frechet mean
        frechet_pol(l, theta_each_l, phi_each_l,  &frechet_vec_theta, &frechet_vec_phi);
...
int main(){
    ...
    int LMAX = 800;
    char *filename = "mvs_lmax800_mc2.dat";
    ...
    double *frechet_theta = (double *)malloc(LMAX * sizeof(double));
    double *frechet_phi = (double *)malloc(LMAX * sizeof(double));

    if (coef_theta == NULL || coef_phi == NULL || frechet_theta == NULL || frechet_phi == NULL) {
    ...
    for (int j = 0; j < COEF_TAMANHO; j++){
        // Read values from the file
        if (fscanf(file, "%lf %lf\n", &coef_theta[j], &coef_phi[j]) != 2) {
    ...
    frechet_vec(coef_theta, coef_phi, frechet_theta, frechet_phi, LMAX);

    // Print all values of coef_theta and coef_phi
    for (int i = 2; i < LMAX; i++) {
         printf("frechet_theta[%d] = %.15lf, frechet_phi[%d] = %.15lf\n", i, frechet_theta[i], i, frechet_phi[i]);
    }
